...
const Ajuda = () => {
      ...
      content: [
        {
          ...
          description: 'Siga estes passos para começar a usar o Painel Político',
          features: [
            'Configure seu perfil e preferências',
            'Importe seus contatos existentes',
            'Configure os canais de comunicação',
            'Explore os relatórios e analytics',
            'Familiarize-se com o CRM'
          ],
          tips: [
            'Complete todos os passos na ordem sugerida',
            'Use os templates prontos para acelerar o processo',
  ...
  const completedSteps = primeirosPassosSteps.filter(step => step.completed).length;
  const progress = (completedSteps / primeirosPassosSteps.length) * 100;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          {/* Primeiros Passos Content */}
          {activeSection === 'primeiros-passos' && (
            <div className="space-y-6">
                ...
                <CardHeader className="pb-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200">
                      {completedSteps}/{primeirosPassosSteps.length} concluído
                    </Badge>
                  </div>
              ...
              {/* Primeiros Passos module content */}
              {sections.find(s => s.id === 'primeiros-passos')?.content.map((item, index) => (
                <Card key={index}>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      {item.title}
          ...
          {filteredSections.filter(section => section.id !== 'primeiros-passos').map((section) => (
            <div
              key={section.id}
